...
PYR_REC=${11:-0.7} # recomposition ratio
...
if [ $PYR_LVL -eq -1 ];
then
	PIXELS=$($DIR/imprintf "%N" $(printf $SEQ $FFR))
...
#	then
		...
		if [ $i -gt $FFR ]; then
			$DECO "$OUT/flt1-"$(printf %03d.tif $((i-$STP))) "$OUT/ma" $PYR_LVL "-flt1-"$(printf %03d.tif $((i-$STP)))
			$DECO "$OUT/flt2-"$(printf %03d.tif $((i-$STP))) "$OUT/ma" $PYR_LVL "-flt2-"$(printf %03d.tif $((i-$STP)))
		fi
		...
		do
			...
			if [ $i -gt $FFR ]; then
	#			F10=$(printf "$OUT/ms%d-flt1-%03d.tif" $l $((i-$STP)))
	#			F20=$(printf "$OUT/ms%d-flt2-%03d.tif" $l $((i-$STP)))
				F10=$(printf "$OUT/ma%d-flt1-%03d.tif" $l $((i-$STP)))
				F20=$(printf "$OUT/ma%d-flt2-%03d.tif" $l $((i-$STP)))
...
#	then
		...
		$DECO "$OUT/smo1-"$(printf %03d.tif $((i+$STP))) "$OUT/ma" $PYR_LVL "-smo1-"$(printf %03d.tif $((i+$STP)))

		for ((l=PYR_LVL-1; l>=0; --l))
		do
			F1=$(printf "$OUT/ms%d-flt2-%03d.tif"  $l $i)
			S1=$(printf "$OUT/ms%d-smo1-%03d.tif"  $l $i)
	#		S0=$(printf "$OUT/ms%d-smo1-%03d.tif"  $l $((i+$STP)))
			S0=$(printf "$OUT/ma%d-smo1-%03d.tif"  $l $((i+$STP)))
			LSIG=$(bc <<< "scale=2; $SIG / ${PYR_DWN}^$l")
