...
do
	m=$(psnr.sh $(printf $SEQ $i) $(printf $OUT/"flt1-%03d.tif" $i) m 0 2>/dev/null)
...
for i in $(seq $((FFR+0)) $LFR);
do
	m=$(psnr.sh $(printf $SEQ $i) $(printf $OUT/"flt2-%03d.tif" $i) m 0 2>/dev/null)
	MM[$i]=$(plambda -c "$m sqrt" 2>/dev/null)
	PP[$i]=$(plambda -c "255 ${MM[$i]} / log10 20 *" 2>/dev/null)
	SS=$(plambda -c "$m $n $SS * + $((n+1)) /" 2>/dev/null)
	n=$((n+1))
...
for i in $(seq $((FFR+0)) $LFR);
do
	m=$(psnr.sh $(printf $SEQ $i) $(printf $OUT/"smo1-%03d.tif" $i) m 0 2>/dev/null)
	MM[$i]=$(plambda -c "$m sqrt" 2>/dev/null)
	PP[$i]=$(plambda -c "255 ${MM[$i]} / log10 20 *" 2>/dev/null)
	SS=$(plambda -c "$m $n $SS * + $((n+1)) /" 2>/dev/null)
	n=$((n+1))
